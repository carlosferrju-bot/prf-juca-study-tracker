...
@media(max-width:650px){.prf-admin-stats{grid-template-columns:1fr}.prf-admin-table th:nth-child(3),.prf-admin-table td:nth-child(3){display:none}}
</style>
<div id="prfJucaAdminModal" aria-hidden="true">
  <div id="prfJucaAdminPanel">
    ...
    <div class="prf-admin-sub">Gerencie as contas que utilizam o PRF JUCA. A conta administradora é protegida.</div>
    ...
    <div style="overflow:auto"><table class="prf-admin-table"><thead><tr><th>Usuário</th><th>E-mail</th><th>Cadastro</th><th>Status</th><th>Ação</th></tr></thead><tbody id="prfAdminUsers"></tbody></table></div>
  </div>
</div>
<script>
(function(){
  ...
  function render(users){
    ...
    active.textContent=users.filter(u=>u.active).length;
    blocked.textContent=users.filter(u=>!u.active).length;
    body.innerHTML=users.map(u=>{
      const admin=u.role==='admin';
      const on=u.active;
      const date=u.createdAt?new Date(u.createdAt).toLocaleDateString('pt-BR'): '—';
      return '<tr><td><strong>'+esc(u.name)+'</strong>'+(admin?' <span class="prf-admin-pill admin">ADMIN</span>':'')+'</td><td>'+esc(u.email)+'</td><td>'+date+'</td><td><span class="prf-admin-pill '+(on?'active':'blocked')+'">'+(on?'LIBERADO':'RESTRINGIDO')+'</span></td><td><button class="prf-admin-action '+(on?'block':'unblock')+'" data-id="'+esc(u.id)+'" '+(admin?'disabled':'')+'>'+ (admin?'Protegido':(on?'Restringir':'Liberar')) +'</button></td></tr>';
    }).join('') || '<tr><td colspan="5">Nenhum usuário cadastrado.</td></tr>';
  }
  async function load(){
    status.textContent='Atualizando lista...';
    try{
      const r=await fetch('/api/admin',{cache:'no-store'}); const d=await r.json();
      if(!r.ok) throw new Error(d.message||'Falha ao carregar usuários.');
      render(d.users||[]); status.textContent='Lista atualizada agora.';
    }catch(e){status.textContent=e.message||'Não foi possível carregar os usuários.';}
  }
  body.addEventListener('click',async e=>{
    const b=e.target.closest('button[data-id]'); if(!b)return;
    const id=b.dataset.id; const restricting=b.classList.contains('block');
    b.disabled=true;
    try{
      const r=await fetch('/api/admin',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:id,active:!restricting})});
      const d=await r.json(); if(!r.ok) throw new Error(d.message||'Não foi possível alterar o usuário.');
      await load();
    }catch(e){alert(e.message||'Erro ao alterar usuário.');b.disabled=false;}
  });
...
#prfJucaUserBar{position:fixed;right:18px;top:14px;z-index:9999;display:flex;align-items:center;gap:8px;background:#071321ee;border:1px solid #29455f;border-radius:12px;padding:7px 9px;box-shadow:0 12px 35px rgba(0,0,0,.53);backdrop-filter:blur(10px);font:12px Arial,Segoe UI,sans-serif;color:#dcecff}
...
(function(){
 ...
 const a=document.getElementById('prfJucaAdminBtn');
 if(a)a.addEventListener('click',()=>window.prfJucaOpenAdmin&&window.prfJucaOpenAdmin());
 if(!b)return;
