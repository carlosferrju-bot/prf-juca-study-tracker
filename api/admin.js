import { configured, missingConfig, currentUser, readUsers, writeUsers, isAdmin, isApproved } from './_auth.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!userId || (typeof active !== 'boolean' && typeof approved !== 'boolean')) return res.status(400).json({ ok: false, message: 'Informe o usuário e a alteração desejada.' });
      ...
      // The current admin UI sends active:true when clicking "Liberar".
      // For a pending account, that action is also the approval.
      if (active === true && target.approved === false) target.approved = true;
